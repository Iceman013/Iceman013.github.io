...
// Clear all pages
function clear() {
    let pages = ["welcome", "host-page", "join-page", "main-game", "join-game"];
    pages.forEach((i) => {
...
async function beginJoin() {
    ...
    let current = await getMessages(joinDetails.code, serverURL);
    let found = false;
    current.forEach((i) => {
        if (i.gameId == joinDetails.code && i.type == "status" && i.data == "start") {
            found = true;
        }
    });
    if (!found) {
